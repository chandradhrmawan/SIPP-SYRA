  ...
  <div class="row">
    <div class="col-xs-12">
      <div class="box">
        <div class="box-header">
          <h3 class="box-title">Master Data Warna</h3>
        </div>  
       ...
       <?php } ?>
        ...
        <div class="box-body">
          <div class="row">
            <div class="col-xs-12">
              <button class="btn btn-primary pull-right" onclick="add_Warna()"><span class="fa fa-plus"></span> Tambah Master Warna</button>
            </div>
          </div>
          <br>
    ...
    $(document).ready(function() { //TAMPIL DATA TABLE SERVER SIDE
      table = $('#table').DataTable({ 
        ...
        "ajax": {
          "url": "<?php echo site_url('Warna/list_data')?>",
          "type": "POST"
    ...
    function add_Warna()
    {
      ...
      $('.modal-title').text('Tambah Warna'); // Set Title to Bootstrap modal title
    }
    ...
    function save()
    {
      ...
      if(save_method == 'add') 
      {
        url = "<?php echo site_url('Warna/ajax_add')?>";
      }
      else
      {
        url = "<?php echo site_url('Warna/ajax_update')?>";
      }
     ...
     function edit_Warna(id)
     {
      ...
      $.ajax({
        url : "<?php echo site_url('Warna/ajax_get_Warna_by_id')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
          $('[name="id_warna"]').val(data.id_warna);
          $('[name="nama_warna"]').val(data.nama_warna);
          

          $('#modal_formView').modal('show'); // show bootstrap modal when complete loaded
          $('.modal-title').text('Update Form Master Warna'); // Set title to Bootstrap modal title
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          ...
          <h3 class="modal-title">Master Warna Form Input</h3>
        </div>
        <form action="#" id="form">
          <div class="modal-body form">
            <div class="row col-md-12 center">
            <input type="hidden" value="" name="id_warna"/> 
             <div class="form-group">
                <label for="nama_warna" class="form-control-label">Nama Warna</label>
                <input name="nama_warna" placeholder="Nama Warna" class="form-control" type="text" required="required">
              </div>
            </div>
          </div>
